...
class AccountMove(models.Model):
	...
	def action_preview_ksef_pdf(self):
		...
		report = self.env.ref('hfb_xmlmap_exporter.action_report_ksef_invoice')
		_logger.info(
			f"\n⚡️ REPORT: {report}"
			f"\n⚡️ REPORT MODEL: {report.model}"
			f"\n⚡️ REPORT NAME: {report.report_name}"
		)

		report_obj = self.env['ir.actions.report']._get_report_from_name(
		    'hfb_xmlmap_exporter.report_ksef_invoice'
		)

		_logger.info(
			f"\n REAL REPORT MODEL: {report_obj.model}"
			f"\n REAL REPORT: {report_obj}"
		)
